// File: pages/admin/analytics/monthly.js
// (BARU - Halaman Grafik Bulanan)
...
export default function MonthlyAnalyticsPage() {
    ...
    useEffect(() => {
        if (status === 'authenticated' && session?.user?.isAdmin) {
            // UBAH: Ambil API bulanan
            fetch('/api/admin/analytics/sales-over-time-monthly') 
                .then(res => res.json())
                .then(data => {
                    setSalesData(data);
                    setLoading(false);
            ...
            <div style={{ width: '100%', height: 400 }}>
                <ResponsiveContainer>
                    <LineChart data={salesData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                        ...
                        <XAxis dataKey="month" stroke="#ccc" /> {/* dataKey adalah 'month' */}
    ...
    return (
        <div className="min-h-screen bg-gray-900 p-8 text-white">
            <div className="flex justify-between items-center mb-8">
                <h1 className="text-3xl font-bold">Omset Bulanan</h1>
                <div>
                    ...
                    <Link href="/admin/analytics/daily" legacyBehavior>
                        <a className="bg-teal-600 hover:bg-teal-700 text-white py-2 px-4 rounded">
                            Lihat Harian
                        </a>
                    </Link>
                </div>
